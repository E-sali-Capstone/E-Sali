...
@extends('dashboard.template.layout', ['thread' => null])
    ...
    <div class="v-category-show">
        <div class="clearfix">
            @if ($category->accepts_threads)
                @can ('createThreads', $category)
                    <a href="{{ Forum::route('thread.create', $category) }}" class="btn btn-primary float-end btn-sm">{{ trans('forum::threads.new_thread') }}</a>
                @endcan
            @endif
            ...
            <div class="btn-group" role="group">
                @can ('manageCategories')
                    ...
                    <button type="button" class="btn btn-secondary btn-rounded btn-sm" data-open-modal="edit-category">
        ...
        @if (! $category->children->isEmpty())
            @foreach ($category->children as $subcategory)
                @include('dashboard.forum.category.partials.list', ['category' => $subcategory])
            @endforeach
        @endif
        ...
        @if ($category->accepts_threads)
            @if (! $threads->isEmpty())
                <div class="mt-4">
                    {{ $threads->links('dashboard.forum.pagination') }}
                </div>
                ...
                <div class="threads list-group my-3 shadow-sm">
                    @foreach ($threads as $thread)
                        @include ('dashboard.forum.thread.partials.list')
                    @endforeach
                </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="mb-3" v-if="selectedAction == 'move'">
                                                ...
                                                <select name="category_id" id="category-id" class="form-select">
                                                    @include ('dashboard.forum.category.partials.options', ['hide' => $category])
                                                </select>
                                            </div>
                                            ...
                                            <div class="text-end">
                                                <button type="submit" class="btn btn-primary btn-rounded btn-sm" @click="submit" :disabled="selectedAction == null">{{ trans('forum::general.proceed') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </transition>
            ...
            <div class="row">
                ...
                <div class="col col-xs-4 text-end">
                    @if ($category->accepts_threads)
                        @can ('createThreads', $category)
                            <a href="{{ Forum::route('thread.create', $category) }}" class="btn btn-primary btn-sm">{{ trans('forum::threads.new_thread') }}</a>
                        @endcan
                    @endif
                </div>
            </div>
    ...
    @if (! $threads->isEmpty())
        @can ('markThreadsAsRead')
            <div class="text-center mt-3">
                <button class="btn btn-primary px-5 btn-rounded btn-sm" data-open-modal="mark-threads-as-read">
                    <i data-feather="book"></i> {{ trans('forum::general.mark_read') }}
                </button>
            </div>

            @include ('dashboard.forum.category.modals.mark-threads-as-read')
        @endcan
    @endif

    @can ('manageCategories')
        @include ('dashboard.forum.category.modals.edit')
        @include ('dashboard.forum.category.modals.delete')
    @endcan
