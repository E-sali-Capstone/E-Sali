<div @if (! $post->trashed())id="post-{{ $post->sequence }}"@endif
    ...
    <div class="card-header">
        ...
        <span class="text-muted">
            @include ('dashboard.forum.partials.timestamp', ['carbon' => $post->created_at])
            @if ($post->hasBeenUpdated())
                ({{ trans('forum::general.last_updated') }} @include ('dashboard.forum.partials.timestamp', ['carbon' => $post->updated_at]))
            @endif
        </span>
    </div>
    <div class="card-body">
        @if ($post->parent !== null)
            @include ('dashboard.forum.post.partials.quote', ['post' => $post->parent])
        @endif
