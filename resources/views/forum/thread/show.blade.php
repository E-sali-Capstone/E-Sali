...
<div class="content-wrapper container">
    ...
    <div id="thread" class="v-thread">
        <div class="d-flex flex-column flex-md-row justify-content-between">
            <h3 class="flex-grow-1">{{ $thread->title }}</h3>

            <div>
                @if (Gate::allows('deleteThreads', $thread->category) && Gate::allows('delete', $thread))
                    @if ($thread->trashed())
                        <a href="#" class="btn btn-danger text-white btn-sm mr-3 mb-2" data-open-modal="perma-delete-thread">
                            <i data-feather="trash"></i> {{ trans('forum::general.perma_delete') }}
                        </a>
                    @else
                        <a href="#" class="btn btn-danger btn-sm text-white mr-3 mb-2" data-open-modal="delete-thread">
                            @if(Auth::user()->id == $thread->author_id || Auth::user()->user_type == "Admin" )
                ...
                @if ($thread->trashed() && Gate::allows('restoreThreads', $thread->category) && Gate::allows('restore', $thread))
                    <a href="#" class="btn btn-secondary mb-2 btn-sm" data-open-modal="restore-thread">
                ...
                @if (Gate::allows('lockThreads', $category)
                    ...
                    <div class="btn-group mb-2 btn-sm" role="group">
                        @if (! $thread->trashed())
                            @can ('lockThreads', $category)
                                @if ($thread->locked)
                                    <a href="#" class="btn btn-secondary btn-sm" data-open-modal="unlock-thread">
                                        <i data-feather="unlock"></i> {{ trans('forum::threads.unlock') }}
                                    </a>
                                @else
                                    <a href="#" class="btn btn-secondary btn-sm" data-open-modal="lock-thread">
                            ...
                            @can ('pinThreads', $category)
                                @if ($thread->pinned)
                                    <a href="#" class="btn btn-secondary btn-sm" data-open-modal="unpin-thread">
                            ...
                            @endcan
                            @can ('rename', $thread)
                                <a href="#" class="btn btn-secondary btn-sm" data-open-modal="rename-thread">
                                    <i data-feather="edit-2"></i> {{ trans('forum::general.rename') }}
                                </a>
                            @endcan
                            @can ('moveThreadsFrom', $category)
                                <a href="#" class="btn btn-secondary btn-sm" data-open-modal="move-thread">
        ...
        <div class="row mb-3">
            <div class="col col-xs-8">
                {{ $posts->links('dashboard.forum.pagination') }}
            </div>
            <div class="col-md-auto text-end">
                @if (! $thread->trashed())
                    @can ('reply', $thread)
                        <div class="btn-group" role="group">
                            <a href="{{ Forum::route('post.create', $thread) }}" class="btn btn-primary btn-sm">
                                {{ trans('forum::general.new_reply') }}
                            </a>
                            <a href="#quick-reply" class="btn btn-primary btn-sm">
        ...
        @foreach ($posts as $post)
            @include ('dashboard.forum.post.partials.list', compact('post'))
        @endforeach
                    ...
                    <transition name="fade">
                        <div class="card text-white bg-secondary shadow-sm" v-if="selectedPosts.length">
                            ...
                            <div class="card-body">
                                ...
                                <div class="text-end">
                                    <button type="submit" class="btn btn-primary btn-rounded btn-sm" @click="submitPosts">{{ trans('forum::general.proceed') }}</button>
                                </div>
                            </div>
                        </div>
                    </transition>
            ...
            @can ('reply', $thread)
                ...
                <div id="quick-reply">
                    <form method="POST" action="{{ Forum::route('post.store', $thread) }}">
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary px-5 btn-rounded btn-sm btn-sm">{{ trans('forum::general.reply') }}</button>
                        </div>
                    </form>
                </div>
            @endcan
    ...
    @if ($thread->trashed() && Gate::allows('restoreThreads', $thread->category) && Gate::allows('restore', $thread))
        @component('dashboard.forum.modal-form')
            ...
            @slot('actions')
                <button type="submit" class="btn btn-primary btn-rounded btn-sm">{{ trans('forum::general.proceed') }}</button>
            @endslot
        @endcomponent
    @endif
    ...
    @if (Gate::allows('deleteThreads', $thread->category) && Gate::allows('delete', $thread))
        @component('dashboard.forum.modal-form')
            ...
            @slot('actions')
                <button type="submit" class="btn btn-danger btn-rounded btn-sm">{{ trans('forum::general.proceed') }}</button>
            @endslot
        @endcomponent
            ...
            @component('dashboard.forum.modal-form')
                ...
                <div class="input-group">
                    ...
                    <select name="category_id" id="category-id" class="form-select">
                        @include ('dashboard.forum.category.partials.options', ['hide' => $thread->category])
                    </select>
                </div>
