<div class="list-group-item {{ $thread->pinned ? 'pinned' : '' }} {{ $thread->locked ? 'locked' : '' }} {{ $thread->trashed() ? 'deleted' : '' }}" :class="{ 'border border-blue-500': selectedThreads.includes({{ $thread->id }}) }">
    <div class="row align-items-center text-center">
        <div class="col-sm text-md-start">
            ...
            <br>
            {{ $thread->authorName }} <span class="text-muted">@include ('dashboard.forum.partials.timestamp', ['carbon' => $thread->created_at])</span>
            <br>
            @if(Auth::user()->user_type ==  "Admin")
                <!-- <a href="{{route('forum-insight' , $thread->id)}}"> <i>View Insights</i> </a> -->
        ...
        @if ($thread->lastPost)
            <div class="col-sm text-md-end text-muted">
                ...
                <span class="text-muted">@include ('dashboard.forum.partials.timestamp', ['carbon' => $thread->lastPost->created_at])</span>
            </div>
        @endif
